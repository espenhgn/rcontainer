...
The remaining codes may then be added to and be modified further to suit the requirements of the `<rcontainer>` project. 
...
Revise the `<rcontainer>/.gitattributes` file as necessary. Some common file formats has been added already.
...
[![Flake8 lint](https://github.com/espenhgn/rcontainer/actions/workflows/python.yml/badge.svg)](https://github.com/espenhgn/rcontainer/actions/workflows/python.yml)
[![Dockerfile lint](https://github.com/espenhgn/rcontainer/actions/workflows/docker.yml/badge.svg)](https://github.com/espenhgn/rcontainer/actions/workflows/docker.yml)
[![Container build](https://github.com/espenhgn/rcontainer/actions/workflows/container_build.yml/badge.svg)](https://github.com/espenhgn/rcontainer/actions/workflows/container_build.yml)
[![Container build push](https://github.com/espenhgn/rcontainer/actions/workflows/container_build_push.yml/badge.svg)](https://github.com/espenhgn/rcontainer/actions/workflows/container_build_push.yml)

## Description of available containers

* ``rcontainer`` - a hello-world introductory container setup
...
git clone --depth 1 https://github.com/espenhgn/rcontainer.git
cd rcontainer
git lfs pull  # pull "large" files
```

#### Update the `rcontainer.sif` container

To obtain updated versions of the Singularity Image Format (.sif) container file `, issue

```bash
cd path/to/repositories/rcontainer/singularity
mv rcontainer.sif rcontainer.sif.old  # optional, just rename the old(er) file
apptainer pull rcontainer.sif docker://ghcr.io/espenhgn/rcontainer:<tag>  # or
singularity pull rcontainer.sif docker://ghcr.io/espenhgn/rcontainer:<tag> # or 
oras pull ghcr.io/espenhgn/rcontainer_sif:<tag>
```

where `<tag>` corresponds to a tag listed under [packages](https://github.com/espenhgn/rcontainer/pkgs/container/rcontainer),
such as `latest`, `main`, or `sha_<GIT SHA>`. 
The `oras pull` statement pulls the `rcontainer.sif` file from [ghcr.io](https://github.com/espenhgn/rcontainer/pkgs/container/rcontainer_sif) using the [ORAS](https://oras.land) registry, without the need to build the container locally.
...
```bash
docker pull ghcr.io/espenhgn/rcontainer:<tag>
```
...
>
>```bash
>docker build --platform=linux/amd64 -t ghcr.io/espenhgn/rcontainer -f dockerfiles/rcontainer/Dockerfile .
>```
...
export IMAGE="ghcr.io/espenhgn/rcontainer:latest"  # adapt as necessary
...
Some secure platforms do not have direct internet access, hence we recommend cloning/pulling all required files on a machine with internet access as explained above, and archive the `rcontainer` directory with all files and moving it using whatever file uploader is available for the platform.

```bash
cd /path/to/rcontainer
SHA=$(git rev-parse --short HEAD)
cd ..
tar --exclude=".git/*" -cvf rcontainer_$SHA.tar rcontainer
```
...
For instructions on how to build or rebuild containers manually using [Docker](https://www.docker.com) and [Singularity](https://docs.sylabs.io) refer to [`<rcontainer>/docker/README.md`](https://github.com/espenhgn/rcontainer/blob/main/docker/README.md).

## Unit tests

Unit tests are available in the `<rcontainer>/tests` directory. 
...
`<rcontainer>/docs/source/_build/html/index.html`

The documentation may also be hosted online on [readthedocs.org](https://readthedocs.org).

## SLURM jobscript example

A basic job script example for running a Singularity container in an HPC setting with the [SLURM](https://slurm.schedmd.com) job scheduler is provided in the file [singularity_slurm_job.sh](https://github.com/espenhgn/rcontainer/blob/main/scripts/singularity_slurm_job.sh), and should be modified as needed.
It expects a few environment variables, and can be submitted as

```
export JOBNAME=rcontainer
...
```
The output of the job will be written to the text files `rcontainer.out` (output) and `rcontainer.err` (errors).


## Workflows

The following sections describe how to run the provided workflows using different workflow management systems.
The Docker container is assumed to be available and tagged as `ghcr.io/espenhgn/rcontainer:latest`: 

```bash
# (optional) make sure that the container is available; tagged as "latest"
docker pull --platform=linux/amd64 ghcr.io/espenhgn/rcontainer:<tag>
docker image tag ghcr.io/espenhgn/rcontainer:0.1.0rc8 ghcr.io/espenhgn/rcontainer:latest
```
...
We have included a basic WDL file and JSON file defining inputs in the `<rcontainer>/wdl` directory.
...
We have provided a basic Snakefile in the `<rcontainer>/snakemake` directory.
...
If you face any issues, or if you need additional software, please let us know by creating a new [issue](https://github.com/espenhgn/rcontainer/issues/new).
